...
const Container = styled.div`
  ...
  background: linear-gradient(135deg, ${theme.colors.bg.dark} 0%, ${theme.colors.primary} 100%);
    ...
    return (
        <Container>
            <LoginCard>
                ...
                <Subtitle>
                    {isRegister ? 'Registrar nuevo administrador' : 'Acceso seguro al sistema'}
                </Subtitle>
                ...
                <Form onSubmit={handleSubmit}>
                    ...
                    <Button type="submit" disabled={loading} style={{ width: '100%', marginTop: theme.spacing.md }}>
                        <FiLogIn size={20} /> {loading ? (isRegister ? 'Procesando...' : 'Verificando...') : (isRegister ? 'Crear Admin' : 'Entrar al Panel')}
                    </Button>
                </Form>

                <ToggleText onClick={() => setIsRegister(v => !v)}>
                    {isRegister ? '¿Ya tienes cuenta? Inicia sesión' : '¿No tienes cuenta? Registrar Admin'}
                </ToggleText>
            </LoginCard>
        </Container>
    )
